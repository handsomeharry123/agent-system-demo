...
for (const t of targets) {
  ...
  let heading = '';
  try {
    const h1 = await page.locator('.ant-page-header-heading-title').first();
    heading = (await h1.innerText({ timeout: 1500 })).trim();
  } catch {}
  if (!heading) {
    try {
      const h2 = await page.locator('h1, h2, h3').first();
      heading = (await h2.innerText({ timeout: 1500 })).trim();
    } catch {}
  }
  if (!heading) {
  ...
  const shotPath = `/tmp/monitoring-verify2-${t.name}.png`;
