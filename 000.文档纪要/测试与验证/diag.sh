...
echo "================ 工作目录 ================"
pwd
ls -la frontend/vite.config.ts frontend/package.json | head -5
...
echo
echo "================ 4) 配置语法快速校验 ================"
node -e "import('./frontend/vite.config.ts').then(m => { console.log('  vite.config.ts: parse OK'); console.log('  server config:', JSON.stringify(m.default.server, null, 2)); }).catch(e => { console.log('  vite.config.ts: PARSE FAIL'); console.error(e.message); process.exit(1); })" 2>&1
