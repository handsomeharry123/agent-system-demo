...
//     · 更多下拉：查看详情 / 编辑 / 禁用 / 查看资源申请 / 创建评测任务 / 查看监控告警
...
const LedgerList = () => {
  ...
  const handleStatusChange = (record: LedgerAgent) => {
    const enabling = record.lifecycleStatus === '已禁用';
    record.lifecycleStatus = enabling ? '已上线' : '已禁用';
    record.runtimeStatus = enabling ? '在线' : '禁用';

    const monitoredAgent =
      mockAgentsV18.find((item) => item.id === record.id) ||
      matchAgentByName(record.name, mockAgentsV18);
    if (monitoredAgent) {
      monitoredAgent.status = enabling ? '在线' : '禁用';
      monitoredAgent.instances.online = enabling
        ? Math.max(1, monitoredAgent.instances.expected)
        : 0;
      monitoredAgent.heartbeatRate = enabling ? 100 : 0;
      ...
      ? [
          {
            ...
            label: '查看详情',
            icon: <EyeOutlined />,
            onClick: () => navigate(`/app/ledger/detail/${record.id}?view=detail`),
          },
          {
            key: 'edit',
            label: '编辑',
            icon: <EditOutlined />,
            onClick: () => navigate(`/app/ledger/detail/${record.id}?view=detail&action=edit`),
          },
          {
            key: record.lifecycleStatus === '已禁用' ? 'enable' : 'disable',
            label: record.lifecycleStatus === '已禁用' ? '启用' : '禁用',
            icon:
              record.lifecycleStatus === '已禁用' ? <PlayCircleOutlined /> : <StopOutlined />,
