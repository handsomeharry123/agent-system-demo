...
const isSystemAdministrator = (user: CenterUser): boolean =>
  user.id === 'user-000' || user.employeeId === 'admin' || user.name === '系统管理员';
...
export const initialCenterUsers: CenterUser[] = mockUsers.slice(0, 12).map((user, index) => {
  const isSystemAdmin = user.id === 'user-000';
  const roles: UserRole[] = user.roles.length ? user.roles : ['科室管理员'];
  return {
    id: user.id,
    name: isSystemAdmin ? '系统管理员' : user.name,
    employeeId: user.employeeId,
    password: user.password || '123456',
    department: user.department,
    phone: user.phone.replace(/\*/g, String((index + 3) % 10)),
    roles,
...
const normalizeCenterUsers = (users: Array<CenterUser & { role?: UserRole }>): CenterUser[] =>
  users.map(({ role, ...user }) => ({
    ...user,
    name: user.id === 'user-000' ? '系统管理员' : user.name,
    employeeId: user.id === 'user-000' ? 'admin' : user.employeeId,
    department: user.id === 'user-000' ? '信息中心' : user.department,
    roles: user.id === 'user-000'
      ? ['信息科管理员']
      : user.roles?.length
        ? user.roles
        : role
...
const UserList = () => {
  ...
  const [filters, setFilters] = useState<{ userId?: string; department?: string; role?: UserRole; status?: AccountStatus }>({});
  ...
  const userOptions = useMemo(
    () => users.map((user) => ({
      label: `${user.name} / ${user.employeeId} / ${user.phone}`,
      value: user.id,
    })),
    [users],
  );
  ...
  const updateStatus = (ids: React.Key[], status: AccountStatus) => {
    const effectiveIds = status === '停用'
      ? ids.filter((id) => !users.some((user) => user.id === id && isSystemAdministrator(user)))
      : ids;
    if (effectiveIds.length === 0) {
      message.warning('系统管理员不支持停用');
      return;
    }
    setUsers((current) => {
      const next = current.map((user) => (effectiveIds.includes(user.id) ? { ...user, status } : user));
      sessionStorage.setItem('user-center-users', JSON.stringify(next));
      return next;
    });
    setSelectedRowKeys([]);
    message.success(`已${status === '正常' ? '启用' : '停用'} ${effectiveIds.length} 个帐号`);
  };

  const confirmBatch = (status: AccountStatus) => {
    const candidateIds = selectedRowKeys.length ? selectedRowKeys : filteredUsers.map((user) => user.id);
    const ids = status === '停用'
      ? candidateIds.filter((id) => !users.some((user) => user.id === id && isSystemAdministrator(user)))
      : candidateIds;
    if (ids.length === 0) {
      message.warning('系统管理员不支持停用');
      return;
  ...
  const deleteUser = (record: CenterUser) => {
    if (isSystemAdministrator(record)) {
      message.warning('系统管理员不支持删除');
      return;
    }
    Modal.confirm({
      title: '确认删除用户？',
      content: `删除后将无法恢复“${record.name}”的用户帐号。`,
      okText: '删除',
      okButtonProps: { danger: true },
      cancelText: '取消',
      onOk: () => {
        setUsers((current) => {
          const next = current.filter((user) => user.id !== record.id);
          sessionStorage.setItem('user-center-users', JSON.stringify(next));
          return next;
        });
  ...
  const columns: ColumnsType<CenterUser> = [
    { title: '用户姓名', dataIndex: 'name', width: 140, fixed: 'left' },
    ...
    { title: '最后登录时间', dataIndex: 'lastLoginAt', width: 170 },
    {
      ...
      render: (_, record) => {
        const protectedAccount = isSystemAdministrator(record);
        return (
          <Space size={2}>
          ...
          <Button
            ...
            title={protectedAccount ? '系统管理员不支持停用' : undefined}
          ...
          <Button
            ...
            danger
            disabled={protectedAccount}
            title={protectedAccount ? '系统管理员不支持删除' : undefined}
            icon={<DeleteOutlined />}
  ...
  return (
    <div className="user-center-page">
      ...
      <Card className="user-center-filter" bordered={false}>
        <Form form={filterForm} layout="inline" onValuesChange={(_, values) => setFilters(values)}>
          ...
          <Form.Item><Button onClick={() => { filterForm.resetFields(); setFilters({}); }}>重置筛选</Button></Form.Item>
        </Form>
      </Card>
      <Card bordered={false}>
        ...
        <Table
          ...
          rowSelection={{
            ...
            getCheckboxProps: (record) => ({
              disabled: isSystemAdministrator(record),
              title: isSystemAdministrator(record) ? '系统管理员不支持停用或删除' : undefined,
            }),
          }}
          scroll={{ x: 1400 }}
